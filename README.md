...
**What I found.** The verdict was unanimous across all four diagnostics: roughly 0.7 market beta, no statistically significant Jensen or factor-neutral alpha, exposures that drift, and modest capacity. This is not one unlucky draw. A signal-ranked, regime-matched, long-only equity sleeve is factor beta *by construction*, so the audit confirms a structural expectation rather than uncovering an accident. That is the honest reason I reframed the page around what the evidence supports: a **cross-asset risk-monitoring and hedge-overlay tool, not an alpha engine**. I state that identity plainly, disclose that the static thesis library carries selection look-ahead (walk-forward validation controls execution look-ahead only), and put one falsifiable call on the record.

*(These numbers are measured on real yfinance / LSEG market returns, not simulated data. The one place data quality bites is dollar-ADV for occasional thin-volume tickers, which the Cost & Capacity panel flags `?data` and excludes.)*

**The product.** The Hedge Overlay is the thing that honesty points at: the exact ETF basket that strips my book's systematic risk, so a parent portfolio can hold the thematic view I intended without the market beta it already owns. I didn't just propose it, I backtested it out of sample. Re-fit on trailing data and applied to unseen months, it removes variance and pulls market beta toward zero, and I keep it honest about the limits (the sector legs over-hedge as exposures drift; the residual has no alpha, so it's exposure control inside a portfolio, not a standalone strategy; and the out-of-sample window is a single macro era, so it shows temporal robustness, not survival through a regime break where the correlation structure inverts).

**Where the edge actually is.** Following that logic to its end: if the stock book is beta and the hedged residual has no alpha, the book is a research artifact, not a return strategy, and I treat it as one. The part of this terminal that carries genuine forward information is not the stock picks. It is the regime and contagion nowcast read as a *risk and volatility* signal, and the hedge overlay that acts on it. So the product is the risk layer; the trade book earns its place as the thing that layer is measured against, not as the pitch.

And I hold the risk layer to the same standard I held the book. The point-in-time test on the [Model Accuracy](#page-by-page-guide) page (Spillover Predictive Power) shows total connectedness leads forward realised volatility with a positive but **modest** edge (a weak Spearman rank correlation, a few points of hit-rate over the coin-flip baseline), while the *directional* call shows no forward-return edge at all, because Diebold-Yilmaz decomposes variance, not returns. So the risk layer is the *better* signal here, not a strong one: honest enough to help size a hedge, not to time a market.
